    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          auto dist_x = circle_center_x - actor_center_x;
          auto dist_y = circle_center_y - actor_center_y;
          
          double dist = sqrt(dist_x*dist_x + dist_y*dist_y);  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = spiral[spiral.size()-1].x - main_goal.location.x;  // <- Update
  auto delta_y = spiral[spiral.size()-1].y - main_goal.location.y;  // <- Update
  auto delta_z = spiral[spiral.size()-1].z - main_goal.location.z;  // <- Update
